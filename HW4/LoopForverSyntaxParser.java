...
public class LoopForeverSyntaxParser {
	...
	public LoopForeverSyntaxParser(laver2 la) {
		this.la = la;
	 ...
	 * */
	public void S() {
		// in the format of S-> S + <EXPR>
		// read next token
		System.out.println("parse " + la.getInputString());
		System.out.println("ENTER S-> S + <EXPR>");
		S();
		la.computeNextToken();
		if (la.getNextToken().equals("ADD_OP")) {			
				la.clearMemory();
				EXPR();			
		} else {
			System.out.println("input rejected by the parser - Lexeme " + la.getNextLexeme() + " not an ADD_OP");			
		}
		System.out.println("EXIT S-> S + <EXPR>\n");
